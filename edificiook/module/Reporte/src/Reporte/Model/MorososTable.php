	...
	*/
	...
	public function crearExcelMorosos($params)
	{
		...
		if($diferenciaDeMeses==0){
			$objSheet->setCellValue($listAbecedario[$currentIndexLetra++]."6", 'DETALLE');
		}
		...
		foreach($dataUnidad as $val){
			$select=$sql->select()
			->from( array('ing'=>'ingreso') )
			->columns( array('ingresoId'=>'ing_in_cod','propietarioId'=>'ing_in_usu','residenteId'=>'ing_in_resi',
					'			mes'=>new Expression("MONTH(ing.ing_da_fecemi)"),
								'year'=>new Expression("YEAR(ing.ing_da_fecemi)"),
								'mes_year'=>new Expression("CONCAT(YEAR(ing.ing_da_fecemi),' ',MONTH(ing.ing_da_fecemi) )") 
							))
			->where(array('ing.uni_in_cod'=>$val['unidadId'],"ing.ing_in_est!=0","ing.ing_da_fecven < '".$this->fechasistema."'") )->group('mes_year')->order(array('year asc','mes asc'));
			...
			if(!empty($rsEmision)){
				...
				while ($i <= $diferenciaDeMeses){
					...
					if($currentMes==(int)$ingresoMes && $currentYear==(int)$ingresoYear){
						...
						$propietarioIdIngreso=$rsEmision[$rsIndex]['propietarioId'];
				...
				if($diferenciaDeMeses==0){
					$currentIndexLetra++;
					$nombreDetalle=($propietarioIdIngreso>0)?$this->getNombreUsuario($propietarioIdIngreso):'';
					$objSheet->setCellValue($listAbecedario[$currentIndexLetra].$indexFila, $nombreDetalle);
				}

				if($deudaTotalUnidad==0){
					$objSheet->removeRow($indexFila);
					$indexFila--;
				}
		...
		$paramsTotales=array(
				...
				'mesIni'=>$mesIni,
				'yearIni'=>$yearIni,
				'diferenciaDeMeses'=>$diferenciaDeMeses
			);

		$listTotalEmitido=$this->getRowTotales('TOTAL EMITIDO',$paramsTotales);	
		$listTotalPagado=$this->getRowTotales('TOTAL COBRADO',$paramsTotales );
		
		$listTotalPendientePago=$this->getRowTotalesPendienteDeCobro($listTotalEmitido,$listTotalPagado);
		$listPorcentajeMorosidad=$this->getPorcentajeMorosidad($listTotalEmitido,$listTotalPendientePago);

		//Escribir total a cobrar.
		$currentIndexLetra=$posColumnaDescripcionTotales;
		$objSheet->setCellValue($listAbecedario[$currentIndexLetra].$indexFila, 'TOTAL POR COBRAR');
		$currentIndexLetra++;

		foreach ($listTotalPendientePago as $value) {
			$objSheet->setCellValue($listAbecedario[$currentIndexLetra].$indexFila, $value);
			$currentIndexLetra++;	
		}
		$indexFila++;


		//Escribir total emitido.
		$currentIndexLetra=$posColumnaDescripcionTotales;
		$objSheet->setCellValue($listAbecedario[$currentIndexLetra].$indexFila, 'TOTAL EMITIDO');
		$currentIndexLetra++;

		foreach ($listTotalEmitido as $value) {
			$objSheet->setCellValue($listAbecedario[$currentIndexLetra].$indexFila, $value);
			$currentIndexLetra++;	
		}
		$indexFila++;
		...
		//Escribir totales porcentaje Morocidad.
		$currentIndexLetra=$posColumnaDescripcionTotales;
		$objSheet->setCellValue($listAbecedario[$currentIndexLetra].$indexFila, '% MOROSIDAD');
		$currentIndexLetra++;

		foreach ($listPorcentajeMorosidad as $value) {
			$objSheet->setCellValue($listAbecedario[$currentIndexLetra].$indexFila, $value);
			$currentIndexLetra++;	
		}
	...
	/**
		* Función genérica que obtiene los totales de emisiones (total emitido, total cobrado).
		*	
		* 	PARAMETROS ENTRADA:
		*	@tipoFila = [ TOTAL EMITIDO | TOTAL COBRADO], 
		*	@params(fechaDesde,fechaHasta,mesIni,yearIni,diferenciaDeMeses).
			
		*	@return array[].
		*/

	private function getRowTotales($tipoFila,$params){

		$fechaDesde=$params['fechaDesde'];
		$fechaHasta=$params['fechaHasta'];
		$mesIni=$params['mesIni'];
		$yearIni=$params['yearIni'];
		$diferenciaDeMeses=$params['diferenciaDeMeses'];
		...
		if($tipoFila=='TOTAL EMITIDO'){
			...
			$select=$sql->select()
				...
				->columns(array(
							'mes'=>new Expression('MONTH(coi.coi_da_fecemi)'),
							'year'=>new Expression('YEAR(coi.coi_da_fecemi)'),
							'totalMes'=>new Expression('SUM(coi.coi_do_subtot)') 
				))
				->join(array('ing'=>'ingreso'),'coi.ing_in_cod=ing.ing_in_cod',array())
				->join(array('uni'=>'unidad'),'uni.uni_in_cod=ing.uni_in_cod',array())
				->where(array('uni.edi_in_cod'=>$this->edificioId,"ing.ing_da_fecven < '".$this->fechasistema."'" ))
				->group(array('year','mes'))->order(array('year asc','mes asc'));
			$select->where->between('ing.ing_da_fecemi',$fechaDesde,$fechaHasta);

		}else if ($tipoFila=='TOTAL COBRADO') {
			$select=$sql->select()
				->from(array('ipa'=>'ingreso_parcial'))
				->columns(array(
						'mes'=>new Expression('MONTH(ipa.ipa_da_fecemi)'),
						'year'=>new Expression('YEAR(ipa.ipa_da_fecemi)'),
						'totalMes'=>new Expression('SUM(ipa.ipa_do_imp)') 
				))
				->join(array('ing'=>'ingreso'),'ipa.ing_in_cod=ing.ing_in_cod',array())
				->join(array('uni'=>'unidad'),'uni.uni_in_cod=ing.uni_in_cod',array())
		...
		$sqlString=$sql->buildSqlString($select);
		$rsEmision=$adapter->query($sqlString,$adapter::QUERY_MODE_EXECUTE)->toArray();
		...
		while ($i <= $diferenciaDeMeses){
			...
			if($currentMes==(int)$ingresoMes && $currentYear==(int) $ingresoYear){
				$emisionMensual=$rsEmision[$rsIndex]['totalMes'];
				$listTotales[]=$emisionMensual;
				$sumaTotalEmision+=$emisionMensual;
				$rsIndex++;
			}else{
				$listTotales[]=0;
			}
		...
		$listTotales[]=$sumaTotalEmision;

		return $listTotales;
	}

	private function getRowTotalesPendienteDeCobro($listTotalEmitido, $listTotalCobrado){
		$total=0;
		$listTotales=array();

		for($i=0;$i<count($listTotalEmitido);$i++){
			if(!empty($listTotalEmitido[$i]) && !empty($listTotalCobrado[$i] ) ) {
		
				$mesTotalPendienteDeCobro=$listTotalEmitido[$i] - $listTotalCobrado[$i];
				
				$listTotales[]=number_format($mesTotalPendienteDeCobro,2,'.','');
				$total+=$mesTotalPendienteDeCobro;
	...
	public function getPorcentajeMorosidad($listTotalEmitido,$listPendienteCobro){
		$total=0;
		$listTotales=array();

		for($i=0;$i<count($listTotalEmitido);$i++){

			if(!empty($listTotalEmitido[$i]) && !empty($listPendienteCobro[$i] ) ) {
		
				$pctMorocidadMes=($listPendienteCobro[$i] * 100) / $listTotalEmitido[$i];
				
				$listTotales[]=round($pctMorocidadMes,2);
