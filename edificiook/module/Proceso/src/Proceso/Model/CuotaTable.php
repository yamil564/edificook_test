...
class CuotaTable extends AbstractTableGateway{
				...
				*/
				...
				if($ingresoId!=null){
					...
					if(false==$this->existeConceptoIngreso(17,$ingresoId)){
						...
						$importeCuotaMant=$this->getCalculoCuotaPorUnidad($currentIdUnidad,'CUOTA DE MANTENIMIENTO');
	...
	private function getAllUnidadesPadresPorEdificio($edificioId){
		...
		$selectUnidadesPadres=$sql->select()
			...
			->where(array('edi_in_cod'=>$edificioId,'uni_in_pad'=>NULL,'uni_in_est'=>1, "uni_vc_tip!='GRUPO' "));
...
//Calculo cuota
	private function getCalculoCuotaPorUnidad($unidadId,$conceptoNombre){
        ...
        $stringSelectSubUnidades="SELECT uni_do_cm2 as porcentaje from unidad WHERE (uni_in_cod=$unidadId and uni_in_est!=0) or (uni_in_pad = $unidadId and uni_in_est!=0)";
		...
		if($this->tipoCalculoCuota=='PROYECTADO'){
			$presupuesto=$this->getPresupuestoDeCuota($conceptoNombre);
			if(!empty($rsUnidades)){
	...
	private function getPresupuestoDeCuota($conceptoNombre='CUOTA DE MANTENIMIENTO'){
		$sql=new Sql($this->adapter);
		$selectPresupuesto=$sql->select()
			...
			->where(array("con.con_vc_des"=>$conceptoNombre,
					'con.emp_in_cod'=>0,
...
//Calculo Agua
	...
	private function getImporteAguaSegunEPS($consumoAguaEnM3){
		...
		*/
		...
		if($this->edificioId==55 || $this->edificioId==74){
			if($consumoAguaEnM3 >=0 && $consumoAguaEnM3<=8){
				$precio=(($consumoAguaEnM3 * 1.745) + ($consumoAguaEnM3 * 0.56)) * 1.18;
			}elseif($consumoAguaEnM3>8){
				$precio=( ($consumoAguaEnM3 * 1.892) + ($consumoAguaEnM3 * 0.607)) * 1.18;
			}
			return $precio;
		}

		/*codigo temporal pora edificio bello horizonte*/

		if($this->edificioId==77){
			$precio=(($consumoAguaEnM3 * 1.445) + ($consumoAguaEnM3 * 0.853)) * 1.18;
			return $precio;
		}


		/*if($this->edificioId==77){
			if($consumoAguaEnM3 >=0 && $consumoAguaEnM3<=10){
				$precio=(($consumoAguaEnM3 * 1.52) + ($consumoAguaEnM3 * 0.52)) * 1.18;
			}elseif($consumoAguaEnM3>10 && $consumoAguaEnM3<=25){
				$precio=(($consumoAguaEnM3 * 1.336) + ($consumoAguaEnM3 * 0.605)) * 1.18;
			}elseif($consumoAguaEnM3>25 && $consumoAguaEnM3<=50){
				$precio=(($consumoAguaEnM3 * 2.956) + ($consumoAguaEnM3 * 1.334)) * 1.18;
			}elseif($consumoAguaEnM3>50 && $consumoAguaEnM3<=1000){
				$precio=(($consumoAguaEnM3 * 5.013) + ($consumoAguaEnM3 * 2.263)) * 1.18;
			}
			return $precio;
		}*/
