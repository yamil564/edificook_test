...
class DepartamentoTable extends AbstractTableGateway{
    ...
    public function addUnidad($params){
		...
		$data = array(
			...
			'uni_in_pro'=>$params['selPropietario_'],
            'uni_in_pos'=>$params['selResidente_'],
    ...
    public function updateUnidad($params){
        ...
        $unidadNombrer = str_replace("-", "", str_replace(".", "", str_replace(",", "", str_replace("/", "", $params['textNombre']))));
    ...
    {   
        ...
        $objSheet->setCellValue('L2', 'DIRECCIÓN');
        ...
        $selectString="SELECT u.uni_in_cod as id ,
                            ...
                            propietario.usu_vc_dir as direccionpropietario,
        ...
        foreach($rowsUnidades as $key=>$value){
            ...
            $objSheet->setCellValue('L'.$indexFila, $value['direccionpropietario']);        
        ...
        $objSheet->getStyle('A2:L2')->getAlignment()->setWrapText(true);
        $styleHeader = array(
            'rgb' => 'FFFFFF',
            'size'=>10,
            'alignment' => array(
        ...
        $objSheet->getStyle('A2:L2')->applyFromArray($styleHeader);
        ...
        //BORDER CELL
        $objSheet->getStyle('A1:L1'.(count($rowsUnidades)+2))
	...
	public function formato($idedificio,$idusuario,$accion,$params) {
		...
		switch ($accion) {
			case 'dowload':
                ...
                $objSheet->SetCellValue('B1', 'COD PAGO');
                $objSheet->setCellValue('C1', 'AREA OCUPADA');
                $objSheet->setCellValue('D1', 'BLOQUE');
                $objSheet->setCellValue('E1', 'TIPO');
                $objSheet->setCellValue('F1', 'USO');
                $objSheet->setCellValue('G1', 'DESCRIPCION');
                $objSheet->setCellValue('H1', 'CUOTA M2');
                $objSheet->setCellValue('I1', 'PORCENTAJE');
		        ...
		        $objSheet->getStyle('A1:I1')->getFont()->getColor()->applyFromArray( array('rgb' => 'FFFFFF') );
