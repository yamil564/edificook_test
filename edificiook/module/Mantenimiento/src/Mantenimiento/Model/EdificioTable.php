...
class EdificioTable extends AbstractTableGateway{
	...
	public function nuevoEdificio($params) {
		$adapter = $this->getAdapter();
		$sql = new Sql($adapter);

		$queryString = "INSERT INTO edificio (edi_vc_des, edi_te_link, edi_te_men, tip_in_cod, emp_in_cod) VALUES ('{$params['nombre']}', '', '', 2, 8)";
		// $result = $adapter->query($queryString, $adapter::QUERY_MODE_EXECUTE)->current();
		$id = $adapter->query($queryString, $adapter::QUERY_MODE_EXECUTE)->getgeneratedValue();

		$queryString = "INSERT INTO usuario_edificio (edi_in_cod, usu_in_cod) VALUES ({$id},{$params['idUsuario']})";
		$uedi = $adapter->query($queryString, $adapter::QUERY_MODE_EXECUTE)->getgeneratedValue();

		$queryString = "SELECT m_in_cod FROM menu WHERE m_vc_ambiente = 'I'";
		$menu = $adapter->query($queryString, $adapter::QUERY_MODE_EXECUTE)->toArray();
		
		foreach($menu as $m){
			$queryInsert = "INSERT INTO permiso (uedi_in_cod, m_in_cod, perm_ch_acciones) VALUES ({$uedi}, {$m['m_in_cod']}, '1-1-1-1-1-1-1')";
			$adapter->query($queryInsert, $adapter::QUERY_MODE_EXECUTE);
		}
        ...
        switch ($accion)
        {
            ...
            case 'cargos-y-otros':
                ...
                try {
                    ...
                    //guardar pdf (archivo extra)
                    if(isset($file['archivoextra']['name']) && $file['archivoextra']['name']!=''){
                        $nombre = $file['archivoextra']['name'];
                        $size = $file['archivoextra']['size'];
                        $temp = $file['archivoextra']['tmp_name'];
                        $type = $file['archivoextra']['type'];

                        $carpetaReglamentoInterno = 'public/file/archivo-extra/'.$idedificio.'-archextra.pdf';
                        copy($temp, $carpetaReglamentoInterno);
                        if(file_exists($carpetaReglamentoInterno)){
                            $this->saveArchivo('edi_vc_archextra',$idedificio,'archextra.pdf');
                        }

                    }else{
                        if($params['delArchivoextra']!='undefined'){
                            $this->deleteArchivo('edi_vc_archextra',$idedificio,$idedificio.'-archextra.pdf');
                        }
                    }
    ...
    {  
        ...
        if($directorio[1]=='rinterno.pdf'){
            $directorio='reglamento-interno';
        }elseif($directorio[1]=='archextra.pdf'){
            $directorio='archivo-extra';
        }else{
            $directorio='manual-operaciones';
        }
        //actualizar archivo    
        $adapter = $this->getAdapter();
        $sql=new Sql($adapter);
