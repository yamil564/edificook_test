...
class DepartamentoTable extends AbstractTableGateway{
    ...
    {   
        ...
        $objSheet->setCellValue('C2', 'RESIDENTE');
        $objSheet->setCellValue('D2', '% PARTICIPACION');
        $objSheet->setCellValue('E2', '% CUOTA');
        $objSheet->setCellValue('F2', 'NUMERO');
        $objSheet->setCellValue('G2', 'PISO');
        $objSheet->setCellValue('H2', 'ASOCIADO A');
        ...
        foreach($rowsUnidades as $key=>$value){
            ...
            $objSheet->setCellValue('C'.$indexFila, $value['residente']);
            $objSheet->setCellValue('D'.$indexFila, $value['pct_participacion']);
            $objSheet->setCellValue('E'.$indexFila, $value['pct_cuota']);
            $objSheet->setCellValue('F'.$indexFila, $value['numero_municipal']);
            $objSheet->setCellValue('G'.$indexFila, $value['piso']);
            $objSheet->setCellValue('H'.$indexFila, $value['unidadpadre']);        
        ...
        $objSheet->getStyle('A2:H2')->getAlignment()->setWrapText(true);
        $styleHeader = array(
            'rgb' => 'FFFFFF',
            'size'=>10,
            'alignment' => array(
        ...
        $objSheet->getStyle('A2:H2')->applyFromArray($styleHeader);

        $listAbecedario=array("A","B","C","D","E","F","G","H","I");
        $listWidthColum=array("A"=>'20',"B"=>"50","C"=>"50",
                            "D"=>"10","E"=>"10","F"=>"10","G"=>"5","H"=>"20");
        for($i=0;$i<8;$i++){
        ...
        //BORDER CELL
        $objSheet->getStyle('A1:H1'.(count($rowsUnidades)+2))
