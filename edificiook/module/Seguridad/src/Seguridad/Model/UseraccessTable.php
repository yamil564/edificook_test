...
* creado por: Meler Carranza, 20/08/2016.
* ultima modificacion por: Meler Carranza
* Fecha Modificacion: 06/09/2016.
* Descripcion: 
*
...
class UseraccessTable extends AbstractTableGateway{
    ...
    public function getUsuariosByEdificio($params) {
        ...
        ->columns(array('id'=>'usu_in_cod',
           ...
           'email'=>'usu_vc_ema',
           'username'=>'usu_vc_usu',
           'perfil'=>'usu_ch_tipoperfil',
           'estado'=>'usu_in_est',
        ))->where(array('usu_in_est!=0'));

        $where=array();
        if (isset($params['currentAmbiente'])){
            if($params['currentAmbiente']=='ue'){
                $where['usu_in_ent']=$params['edificioId'];
            }

            $where['usu_ch_tipoperfil']=$params['currentAmbiente'];
    
        }else{
            $where['usu_ch_tipoperfil']='ue';
        }

        $selectUser->where($where);
    ...
    public function getMenuPorTipoAmbiente($params){
        ...
        $i=0;
       	while ($i <= $totalOpcionesMenu) {
       		...
       		$arrayMenu[$indexNuevoGrupoMenu]=array('grupo'=>ucfirst($currentGrupoMenu),"mgId"=>$rsMenu[$i]['mgId']);
            ...
            //Solamente cuando es 1 registro seleccionado, verificamos las opciones que tiene activado.
            if(count($params['ids'])==1){
                $idUsuario=$params['ids'][0];
                $arraySubMenu=$this->pintarPermisosActivos($arraySubMenu, $idUsuario, $params['edificioId']);
            }
    ...
    private function pintarPermisosActivos($listaMenu,$idUsuario,$idEdificio){
        $adapter=$this->getAdapter();
        $sql=new Sql($this->adapter);

        //Obtenemos el id usuarioEdificio.

        $selectString="SELECT uedi_in_cod as id from usuario_edificio where edi_in_cod=$idEdificio and usu_in_cod=$idUsuario limit 1";
        $rsUsuarioEdificio=$adapter->query($selectString,$adapter::QUERY_MODE_EXECUTE)->current();
        $idUsuarioEdificio=$rsUsuarioEdificio['id'];

        if(empty($idUsuarioEdificio)){
            return $listaMenu;
        }

        //Consulta opciones activadas que tiene el usuario.
        $selectString="SELECT m_in_cod as id from permiso where uedi_in_cod=$idUsuarioEdificio";
        $listaPermisosActivos=$adapter->query($selectString,$adapter::QUERY_MODE_EXECUTE)->toArray();
        
        if( !function_exists( 'array_column' ) ){
          $listaPermisosActivos=$this->array_column1($listaPermisosActivos,'id');
        }else{
          $listaPermisosActivos=array_column($listaPermisosActivos,'id');
        }
        
        
        $i=0;
        $resultListaMenu=$listaMenu;
        foreach ($listaMenu as $item) {
            if(in_array($item['id'], $listaPermisosActivos)){
                $resultListaMenu[$i]['activo']=true;
            }else{
                $resultListaMenu[$i]['activo']=false;
            }

            $i++;
        }

        return $resultListaMenu;
    }
    ...
    public function recordarDatos($params){
        ...
        foreach ($usuarioIds as $userId) {
            ...
            $queryUsuario = "SELECT usu_vc_ema, usu_vc_nom, usu_vc_ape, usu_ch_tip, usu_vc_usu, usu_vc_pasvis  FROM usuario WHERE usu_in_cod = '$userId' 
              and usu_vc_ema!='' and usu_vc_usu!='' and usu_vc_pasvis!='' and usu_in_est!=0";
            ...
            if(!empty($rowUsuario)){
                              ...
                              <tbody>
                                <tr>
                                  <td style="padding-top:40px;padding-bottom:40px;padding-left:20px;padding-right:20px" align="left">
                                    ...
                                    <font face="Arial, sans-serif" color="#333333" style="font-size:14px;line-height:20px">
                                    <strong>URL de acceso al sistema: </strong><a href="http://sys.adinco.pe/">http://sys.adinco.pe/</a>
                                    </font><br><br><br>
                ...
                if ($destinatario != '' && filter_var($destinatario, FILTER_VALIDATE_EMAIL)) {
					...
					$this->enviarcorreodeprueba($destinatario, $titulo, $msg, $remite, $nombreDestinatario, $edificioId);
                }
            }
        }
	...
	public function enviarcorreodeprueba($destinatario, $titulo, $msg, $remite, $nombreDestinatario, $edificioId) {
    ...
    function array_column1( array $input, $column_key, $index_key = null ) {
