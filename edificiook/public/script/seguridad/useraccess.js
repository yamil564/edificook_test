var GridUsuario={
    ...
    init:function(){
        $(this.gridId).jqGrid({
            ...
            datatype: "json",
            mtype: "POST",
            postData:{'currentAmbiente':'ue'},
            colNames:['id',"Normbre","Correo","Usuario","Login","Ultimo Acceso"],
            colModel:[
                ...
                {name:'username',index:'username', width:150,frozen:true,search:false},
                {name:'estado',index:'estado', width:100,frozen:true,search:false,formatter:GridUsuario.formatearEstadoLogin},
                {name:'ultimoacceso',index:'ultimoacceso', width:100,align:"right",sortable: false,search:false,hidden:true},
            ],
            shrinkToFit: true,
            rowList: [],
            pgbuttons: false,
    ...
    formatearEstadoLogin:function(cellvalue,options,rowObject){
        if(cellvalue=='activo'){
            return "<span cell-id='1' title='Activo' class='pointer green'><i class='ace-icon fa fa-lock bigger-160'></i></span>";
        }
        if(cellvalue=='desactivado'){
            return "<span cell-id='1' title='Desactivado' class='pointer red'><i class='ace-icon fa fa-lock bigger-160'></i></span>";
        }
        if(cellvalue=='pendiente'){
            return "<span cell-id='1' title='Perfil indefinido' class='pointer orange'><i class='ace-icon fa fa-lock bigger-160'></i></span>";
        }
...
var Action= {
    ...
    readInfo:function(){
        ...
        var menuName=null;
        var htmlMenu='';
        var ids=$(GridUsuario.gridId).jqGrid('getGridParam','selarrrow'); 
        ...
        $.post(this.readAction,{
            ...
            $.each(data,function(index){
                ...
                htmlMenu+='<div class="col-xs-12"><div class="item-group"><a href="javascript:void(0);" data-toggle="collapse" data-target="#ls_'+grupoId+'" class="icon-list fa fa-plus"></a>'
                ...
                $.each(grupoRows,function(subIndex) {
                    ...
                    var estadoCheck=' ';
                    if(grupoRows[subIndex]['activo']){
                        estadoCheck='checked="true" ';
                    }
                    
                    htmlMenu+='<li class="list-group-item"><input type="checkbox" class="items ace mg_'+grupoId+'" '+estadoCheck+' value="'+menuId+'" /><span class="lbl">'+menuName+'</span></li>';
...
$("#viewUsuariosInternos").click(function(e){
    $("#subtitleIngreso").html('Internos');
    GridUsuario.reloadGrid({'currentAmbiente':'ua'});
    Action.currentAmbiente='I',
    e.preventDefault();
});

$("#viewUsuariosExternos").click(function(e){
    $("#subtitleIngreso").html('Externos');
    GridUsuario.reloadGrid({'currentAmbiente':'ue'});
    Action.currentAmbiente='E',
    e.preventDefault();
});
